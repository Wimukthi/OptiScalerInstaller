...
Friend Partial Class frmUpdate
    ...
    Private Sub LaunchUpdateScript(sourceDir As String, targetDir As String, exeName As String, tempRoot As String)
        ...
        Dim backupDir As String = Path.Combine(tempRoot, "backup")
        Dim logPath As String = Path.Combine(tempRoot, "update.log")
        ...
        sb.AppendLine(">""%LOG%"" echo OptiScaler Installer update log")
        sb.AppendLine(":wait")
        ...
        sb.AppendLine(")")
        sb.AppendLine("if exist ""%BACKUP%"" rd /s /q ""%BACKUP%""")
        sb.AppendLine("mkdir ""%BACKUP%""")
        sb.AppendLine("robocopy ""%TARGET%"" ""%BACKUP%"" /E /COPY:DAT /R:2 /W:1 /NFL /NDL /NJH /NJS /NP >>""%LOG%""")
        sb.AppendLine("set ""RC=%ERRORLEVEL%""")
        sb.AppendLine("if %RC% GEQ 8 (")
        sb.AppendLine("  echo Backup failed with error %RC%. >>""%LOG%""")
        sb.AppendLine("  goto launch")
        sb.AppendLine(")")
        sb.AppendLine("robocopy ""%SOURCE%"" ""%TARGET%"" /E /COPY:DAT /R:2 /W:1 /NFL /NDL /NJH /NJS /NP >>""%LOG%""")
        sb.AppendLine("set ""RC=%ERRORLEVEL%""")
        sb.AppendLine("if %RC% GEQ 8 (")
        sb.AppendLine("  echo Update copy failed with error %RC%. Restoring backup. >>""%LOG%""")
        sb.AppendLine("  robocopy ""%BACKUP%"" ""%TARGET%"" /E /COPY:DAT /R:2 /W:1 /NFL /NDL /NJH /NJS /NP >>""%LOG%""")
        sb.AppendLine(")")
